...
* 自動為網頁加上alt
    * 以程式檢視網頁中沒有被加入alt的標籤，或是不符合 WCAG 2.1 AA 級無障礙標準的alt敘述的標籤，以AI 套件來生成該圖片的alt敘述。
...
### AI自動生成ALT
如使用自動生成ALT功能時，以 Prompt 形式請求AI套件辨識該相片後生成適合的ALT敘述。
Prompt：「
>##任務與目的  
>*為這份網頁檔案加入符合 WCAG 2.1 AA 級無障礙標準的圖片alt敘述。  
>*使用影像辨識，為每一圖片生成一個合適的alt敘述，並寫入img標籤中的alt屬性。  
>##步驟  
>###第一步
>閱讀HTML檔案，找到沒有alt屬性、alt屬性為空、alt屬性的值不符合WCAG 2.1標準的img標籤。  
>###第二步  
>取得前一步所找到的img標籤的圖片，為每一圖片個別生成合適的alt敘述。  
>###第三步  
>將前一步所生成的alt敘述寫進該alt標籤
>##輸出
>將修改後的HTML檔案傳回。
」請使用此 Prompt，並隨附需要自動生成alt的HTML檔案。
